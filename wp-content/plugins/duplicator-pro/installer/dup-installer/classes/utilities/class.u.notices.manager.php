...
final class DUPX_NOTICE_MANAGER
{
    ...
    {
        if (class_exists('DUPX_LOG', false)) {
            DUPX_LOG::info('SAVE NOTICES', DUPX_Log::LV_DEBUG);
        }
        $notices = array(
            'globalData'  => array(
    ...
    private function loadNotices()
    {
        if (file_exists($this->persistanceFile)) {
            if (class_exists('DUPX_LOG', false)) {
                DUPX_LOG::info('LOAD NOTICES', DUPX_Log::LV_DEBUG);
            }
    ...
    {
        if (class_exists('DUPX_LOG', false)) {
            DUPX_LOG::info('RESET NOTICES', DUPX_Log::LV_DEBUG);
        }
        $this->nextStepNotices   = array();
        $this->finalReporNotices = array();
    ...
    {
        if (!empty($this->nextStepNotices) && class_exists('DUPX_LOG', false)) {
            if ($title) {
                DUPX_Log::info(
                    '===================================='."\n".
                    'NEXT STEP NOTICES'."\n".
    ...
    {
        if (!empty($this->finalReporNotices) && class_exists('DUPX_LOG', false)) {
            DUPX_Log::info(
                '===================================='."\n".
