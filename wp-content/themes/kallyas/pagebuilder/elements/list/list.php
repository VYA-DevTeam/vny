...
class ZnList extends ZnElements
{
	...
	function css(){
		...
		$list_hover_color = $this->opt('list_text_hover', false );
		if ( ! empty( $list_hover_color ) ){
			$css .= '.'.$uid.' .znListItems-text:hover{ color:'.$list_hover_color.'}';
		}
		...
		$options = array(
			...
			'general' => array(
				...
				'options' => array(
					...
					array (
						"name"        => __( "List items hover color", 'zn_framework' ),
						"description" => __( "Select a color for the hover effect on list items", 'zn_framework' ),
						"id"          => "list_text_hover",
						"std"         => "",
						"type"        => "colorpicker",
					),
