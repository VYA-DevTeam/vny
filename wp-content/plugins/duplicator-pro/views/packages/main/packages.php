...
if (isset($_SERVER['AUTH_TYPE']) && $_SERVER['AUTH_TYPE'] == 'Basic' && !$global->basic_auth_enabled) {
    $recommended_text_fix_present = true;
    $system_global->add_recommended_quick_fix('Set authentication username and password', 'Automatically set basic auth set username and password',
                                                array(
                                                    'special' => array(
                                                                    'set_basic_auth' => 1
                                                                )
...
if (isset($_GET['dpro_show_error'])) {
    ...
    $system_global->add_recommended_quick_fix('Activate DUP Archive', 'TEST: Switch to <i><b>DUP</b></i> archive. Click on button to fix this!', 
                                                    array(
                                                        'global' => array(
    ...
    $system_global->add_recommended_quick_fix('Activate ZIP Archive', 'TEST: Switch to <i><b>ZIP</b></i> archive. Click on button to fix this!',
                                                    array(
                                                        'global' => array(
    ...
    $system_global->add_recommended_quick_fix('Activate SHELL Archive', 'TEST: Switch to <i><b>Shell ZIP</b></i> archive. Click on button to fix this!',
                                                    array(
                                                        'global' => array(
...
<?php $alert3->showAlert(); ?>
        ...
        $('#dup-pro-fixes').on('click', '.dup-pro-quick-fix',  function() {
            var $this = $(this);
                    pharams = JSON.parse($this.attr('data-param')),
