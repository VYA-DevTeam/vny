...
class DUP_PRO_UI_Alert
{
        ...
        if ($on_licensing_tab === false) {
            if (!file_exists(DUPLICATOR_PRO_SSDIR_PATH."/ovr.dup")) {
                ...
                if ($license_status === DUP_PRO_License_Status::Expired) {
                    $expired_licence_notice_dismiss_time = get_option(DUP_PRO_UI_Notice::OPTION_KEY_EXPIRED_LICENCE_NOTICE_DISMISS_TIME, false);
                    if (false === $expired_licence_notice_dismiss_time || (time() - $expired_licence_notice_dismiss_time) > (DAY_IN_SECONDS * DUP_PRO_UI_Notice::EXPIRED_LICENCE_NOTICE_DISMISS_FOR_DAYS)) {
                        self::showExpired();
                    }
    ...
    private static function showExpired()
    {
        ...
        DUP_PRO_UI_Notice::displayGeneralAdminNotice($htmlMsg, DUP_PRO_UI_Notice::GEN_ERROR_NOTICE, true,
                                                            array(
                                                                'duplicator-pro-admin-notice',
                                                                'dpro-admin-notice'
                                                            ),
                                                            array(
                                                                'data-to-dismiss' => DUP_PRO_UI_Notice::OPTION_KEY_EXPIRED_LICENCE_NOTICE_DISMISS_TIME
                                                            )
                                                    );
    }
