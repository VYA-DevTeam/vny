...
final class DUPX_Ctrl_Params
{
    ...
    public static function setParamsStep1()
    {
        ...
        $paramsManager->setValue(DUPX_Paramas_Manager::PARAM_REPLACE_ENGINE, DUPX_Paramas_Descriptor_engines::getReplaceEngineModeFromParams());
        $paramsManager->setValue(DUPX_Paramas_Manager::PARAM_DB_CHUNK, DUPX_Paramas_Descriptor_engines::getDbChunkFromParams());
        ...
        if ($archive_config->isNetwork()) {
            if ($paramsManager->getValue(DUPX_Paramas_Manager::PARAM_MULTISITE_INST_TYPE) == -1) {
                self::$paramsValidated = false;
                $paramsManager->addParamValidationFaliedNotice(DUPX_Paramas_Manager::PARAM_MULTISITE_INST_TYPE);
            }
        }
    ...
    public static function setParamsAfterValidation()
    {
        ...
        $paramsManager->setValue(DUPX_Paramas_Manager::PARAM_WP_ADDON_SITES_PATHS, DUPX_Validation_test_addon_sites::getAddonsListsFolders());
        DUPX_Paramas_Descriptor_database::updateCharsetAndCollateByDatabaseSettings();

        $configsChecks = DUPX_Validation_test_iswritable_configs::configsWritableChecks();

        if ($configsChecks['wpconfig'] === false) {
            DUPX_LOG::info("WP-CONFIG ISN\'T READABLE SO SET noting ON ".DUPX_Paramas_Manager::PARAM_WP_CONFIG.' PARAM');
        ...
        if ($configsChecks['htaccess'] === false) {
            DUPX_LOG::info("HTACCESS ISN\'T READABLE SO SET noting ON ".DUPX_Paramas_Manager::PARAM_HTACCESS_CONFIG.' PARAM');
        ...
        if ($configsChecks['other'] === false) {
            DUPX_LOG::info("OTHER CONFIGS ISN\'T READABLE SO SET noting ON ".DUPX_Paramas_Manager::PARAM_OTHER_CONFIG.' PARAM');
    ...
    protected static function setParamsDatabase()
    {
        ...
        if ($paramsManager->setValue(DUPX_Paramas_Manager::PARAM_DB_TABLES, DUPX_DB_Tables::getInstance()->getDefaultParamValue()) === false) {
            self::$paramsValidated = false;
        }
    ...
    {
        $paramsManager = DUPX_Paramas_Manager::getInstance();
        if ($paramsManager->setValueFromInput(DUPX_Paramas_Manager::PARAM_AUTO_CLEAN_INSTALLER_FILES, DUPX_Param_item_form::INPUT_POST, false, true) === false) {
            self::$paramsValidated = false;
        }
        $paramsManager->save();
        return self::$paramsValidated;
