...
final class DUPX_S_R_MANAGER
{
    ...
    public function addItem($search, $replace, $type = DUPX_S_R_ITEM::TYPE_STRING, $prority = 10, $scope = true)
    {
        ...
        DUPX_Log::info(
            ...
            'Search:"'.$search.'" Replace:"'.$replace.'" Type:"'.$type.'" Prority:"'.$prority.'" Scope:"'.DUPX_Log::varToString($scope), DUPX_Log::LV_DETAILED);
    ...
    public function getSearchReplaceList($scope = null, $unique_search = true, $globalScope = true)
    {
        ...
        DUPX_Log::info('--- BASE STRINGS ---');
        foreach ($items_list as $index => $item) {
            DUPX_Log::info(
                'SEARCH['.str_pad($item->type, 5, ' ', STR_PAD_RIGHT).']'.str_pad($index + 1, 3, ' ', STR_PAD_LEFT).":".
                str_pad(DUPX_Log::varToString($item->search)." ", 50, '=', STR_PAD_RIGHT).
                "=> ".
                DUPX_Log::varToString($item->replace));
        }
        ...
        $result = array_filter($result, function ($val) {
            if (!empty($val['search'])) {
                ...
                DUPX_Log::info('Empty search string remove, replace: '.DUPX_Log::varToString($val['replace']), DUPX_Log::LV_DETAILED);
                return false;
            }
        });

        if (DUPX_Log::isLevel(DUPX_Log::LV_DEBUG)) {
            DUPX_Log::info('--- REXEXES ---');
...
class DUPX_S_R_ITEM
{
    ...
    public static function searchReplaceWithEncodings($search, $replace, $json = true, $urlencode = true)
    {
        ...
        // JSON ENCODE
        if ($json) {
            $search_json  = DupProSnapJsonU::getJsonWithoutQuotes($search);
            $replace_json = DupProSnapJsonU::getJsonWithoutQuotes($replace);
    ...
    public static function searchReplaceUrl($search_url, $replace_url, $force_new_protocol = true, $normalizeWww = false)
    {
        ...
        if ($normalizeWww && self::domainCanNormalized($search_url)) {
            ...
            $regExSearchUrlJson   = '\\\\\/\\\\\/(?:www\.)?'.preg_quote(DupProSnapJsonU::getJsonWithoutQuotes($baseSearchUrl), '/');
            ...
            $regExSearchUrlJson   = '\\\\\/\\\\\/'.preg_quote(DupProSnapJsonU::getJsonWithoutQuotes($search_url_raw), '/');
        ...
        if ($force_new_protocol && $parse_replace_url !== false && isset($parse_replace_url['scheme'])) {
            ...
            $result[] = array(
                ...
                'replace' => addcslashes(DupProSnapJsonU::getJsonWithoutQuotes('//'.$replace_url_raw), '\\$').'$1'
            );

            $result[] = array(
                'search'  => '/https?:'.$regExSearchUrlJson.self::URL_END_REGEX_MATCH_JSON.'/m',
                'replace' => addcslashes(DupProSnapJsonU::getJsonWithoutQuotes($replace_url), '\\$').'$1'
            );
    ...
    public static function searchReplacePath($search_path, $replace_path)
    {
        ...
        $normaSearchArray = array_map(function ($val) {
            return preg_quote(DupProSnapJsonU::getJsonWithoutQuotes($val), '/');
        }, $explodeSearch);
        ...
        $result[]       = array('search' => $jsonPathSearch, 'replace' => addcslashes(DupProSnapJsonU::getJsonWithoutQuotes($replace_path), '\\$').'$1');
