...
class DAWS_Logger extends DupArchiveLoggerBase
{
    ...
    {
        DUPX_Log::info($s, $flush, $callingFunctionOverride);
    }
    ...
    {
        DUPX_Log::info("ERROR $errno, $errstr, {$errfile}:{$errline}");
        /**
...
class DAWS
{
    ...
    public function processRequest($params)
    {
        ...
        if ($action == 'start_expand') {
            DUPX_Log::info('DAWN START EXPAND');
            ...
            $fileModeOverride      = DupProSnapLibUtil::getArrayValue($params, 'file_mode_override', false, 0644);
            $directoryModeOverride = DupProSnapLibUtil::getArrayValue($params, 'dir_mode_override', false, 0755);

            $action = 'expand';
        } else {
            DUPX_Log::info('DAWN CONTINUE EXPAND');
        }
        ...
        if ($action == 'expand') {
            ...
            if (!$expandState->working) {
                ...
                DUPX_Log::info("DAWN EXPAND DONE, SECONDS: ".$deltaTime, DUPX_Log::LV_DETAILED);

                if (count($expandState->failures) > 0) {
                    DUPX_Log::info('DAWN EXPAND ERRORS DETECTED');

                    foreach ($expandState->failures as $failure) {
                        DUPX_Log::info("{$failure->subject}:{$failure->description}");
                ...
                DUPX_Log::info("DAWN EXPAND CONTINUE", DUPX_Log::LV_DETAILED);
            }
