...
if (!class_exists('DupProSnapLibOrigFileManager', false)) {
     ...
     *
     */
    class DupProSnapLibOrigFileManager
    {
        ...
        {
            $this->rootPath        = DupProSnapLibIOU::safePathUntrailingslashit($root, true);
            $this->origFilesFolder = DupProSnapLibIOU::safePathTrailingslashit($origFolderParentPath, true).self::ORIG_FOLDER_PREFIX.$hash;
        ...
        public function createMainFolder($reset = false)
        {
            ...
            if (!file_exists($this->origFilesFolder)) {
                if (!DupProSnapLibIOU::mkdir($this->origFilesFolder, 'u+rwx')) {
                    throw new Exception('Can\'t create the original files folder '.DupProSnapLibLogger::varToString($this->origFilesFolder));
                }
            }
...
HTACCESS;
        ...
        {
            if (!file_exists($this->origFilesFolder)) {
                throw new Exception('Can\'t get the original files folder '.DupProSnapLibLogger::varToString($this->origFilesFolder));
            }
        ...
        public function deleteMainFolder()
        {
            if (file_exists($this->origFilesFolder) && !DupProSnapLibIOU::rrmdir($this->origFilesFolder)) {
                throw new Exception('Can\'t delete the original files folder '.DupProSnapLibLogger::varToString($this->origFilesFolder));
            }
        ...
        public function addEntry($identifier, $path, $mode = self::MODE_MOVE, $rename = false)
        {
            ...
            if (($relativePath = DupProSnapLibIOU::getRelativePath($path, $this->rootPath)) === false) {
                $isRelative = false;
            } else {
                $isRelative = true;
            }
            $parentFolder = $isRelative ? dirname($relativePath) : DupProSnapLibIOU::removeRootPath(dirname($path));
            ...
            switch ($mode) {
                case self::MODE_MOVE:
                    ...
                    if (!DupProSnapLibIOU::rcopy($path, $dest)) {
                        throw new Exception('Can\'t copy the original file  '.DupProSnapLibLogger::varToString($path));
                    }
                    if (!DupProSnapLibIOU::rrmdir($path, $dest)) {
                        throw new Exception('Can\'t remove the original file  '.DupProSnapLibLogger::varToString($path));
                    }
                    break;
                case self::MODE_COPY:
                    if (!DupProSnapLibIOU::rcopy($path, $dest)) {
        ...
        public function restoreEntry($identifier, $save = true, $defaultIfIsntRelative = null)
        {
            ...
            switch ($this->origFolderEntries[$identifier]['mode']) {
                case self::MODE_MOVE:
                    if (!DupProSnapLibIOU::rename($stored, $original)) {
                        throw new Exception('Can\'t move the original file  '.DupProSnapLibLogger::varToString($stored));
                    }
                    break;
                case self::MODE_COPY:
                    if (!DupProSnapLibIOU::rrmdir($stored)) {
                        throw new Exception('Can\'t delete entry '.DupProSnapLibLogger::varToString($stored));
                    }
                    break;
                default:
                    throw new Exception('invalid mode addEntry');
