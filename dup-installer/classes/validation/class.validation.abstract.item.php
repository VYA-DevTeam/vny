...
abstract class DUPX_Validation_abstract_item
{
    ...
    public function test($reset = false)
    {
        if ($reset || is_null($this->testResult)) {
            try {
                DUPX_Log::resetTime(DUPX_Log::LV_DEBUG);
                DUPX_Log::info('START TEST "'.$this->getTitle().'" [CLASS: '.get_called_class().']');
                $this->testResult = $this->runTest();
            }
            catch (Exception $e) {
                DUPX_Log::logException($e, DUPX_Log::LV_DEFAULT, '      TEST "'.$this->getTitle().'" EXCEPTION:');
            ...
            DUPX_Log::logTime('TEST "'.$this->getTitle().'" RESULT: '.$this->resultString()."\n", DUPX_Log::LV_DEFAULT, false);
        }
    ...
    public function getContent()
    {
        try {
            ...
            DUPX_Log::logException($e, DUPX_Log::LV_DEFAULT, 'VALIDATION DISPLAY CONTENT '.get_called_class().' RESULT: '.$this->resultString().' EXCEPTION:');
            ...
            DUPX_Log::logException($e, DUPX_Log::LV_DEFAULT, 'VALIDATION DISPLAY CONTENT '.get_called_class().' ERROR:');
            return 'DISPLAY CONTENT PROBLEM <br>'
                .'MESSAGE: '.$e->getMessage().'<br>'
                .'TRACE:'
                .'<pre>'.$e->getTraceAsString().'</pre>';
