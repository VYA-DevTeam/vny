...
if (!$isRecoveable) {
    ...
    $templareRecoveryAlter->title   = (isset($schedule) ? __('Schedule: Recovery Status', 'duplicator-pro') : __('Template: Recovery Status', 'duplicator-pro'));
    $templareRecoveryAlter->width   = 700;
    $templareRecoveryAlter->height  = 570;
    $templareRecoveryAlter->message = DupProSnapLibIOU::getInclude(
            __DIR__ . '/template-filters-info.php', array(
            'template'     => $template,
...
<span class="dup-template-recoveable-info-wrapper" >
    ...
    if ($isRecoveable) {
        ...
        ?>
        <span id="dup-template-recoveable-info-<?php echo $templareRecoveryAlter->getUniqueIdCounter(); ?>" class="dup-template-recoveable-info disabled maroon" >
