...
class DUPX_DB_Functions
{
        ...
        if (is_null($this->dbh)) {
            ...
            if (($dbh = DUPX_DB::connect($dbhost, $dbuser, $dbpass, $dbname)) == false) {
                ...
                $msg         = "Unable to connect with the following parameters:<br/>"
                    ."HOST: ".DUPX_Log::varToString($dbhost)."\n"
                    ."DBUSER: ".DUPX_Log::varToString($dbuser)."\n"
                    ."DATABASE: ".DUPX_Log::varToString($dbname)."\n"
                    ."MESSAGE: ".$dbConnError;
                DUPX_Log::error($msg);
            } else {
                $this->dbh            = $dbh;
    ...
    public function getRealCharsetByParam()
    {
        ...
        $sourceCharset = DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_DB_CHARSET);
    ...
    public function getRealCollateByParam()
    {
        ...
        $sourceCollate = DUPX_Paramas_Manager::getInstance()->getValue(DUPX_Paramas_Manager::PARAM_DB_COLLATE);
    ...
    public function getLegacyCharsetAndCollationRegexes()
    {
        ...
        if (is_null($regResult)) {
            ...
            if (count($regResult['search']) > 0) {
                DUPX_Log::info('LEGACY CHARSET REPLACE');

                for ($i = 0; $i < count($regResult['search']); $i++) {
                    DUPX_Log::info('SEARCH  '.$regResult['search'][$i]);
                    DUPX_Log::info('REPLACE '.$regResult['replace'][$i]);
                }
            } else {
                $regResult = false;
            }
