...
require_once(DUPX_INIT.'/classes/params/descriptors/class.u.params.descriptors.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.pass.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.plugins.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.tables.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.wpconfig.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.urlmapping.php');
require_once(DUPX_INIT.'/classes/params/items/class.u.param.item.form.users.pass.reset.php');
...
final class DUPX_Paramas_Manager
{
    ...
    const PARAM_RESTORE_BACKUP_MODE                    = 'restore-backup';
    ...
    public function initParams()
    {
        ...
        DUPX_Paramas_Descriptors::init($this->params);
        return true;
    }
    ...
    {
        if (!isset($this->params[$key])) {
            throw new Exception('Param key '.DUPX_Log::varToString($key).'does not exists');
        }
    ...
    {
        DUPX_Log::info('OVERWRITE PARAMS');
        ...
        if (!empty($arrayData)) {
            ...
            DUPX_Log::info(' *** FROM PACKAGE');
            if ($this->setParamsValues($arrayData, true, DUPX_Log::LV_DEFAULT) === false) {
        ...
        DUPX_Paramas_Descriptors::updateParamsAfterOverwrite($this->params);
        DUPX_Log::info("********************************************************************************");
        return true;
    }
     ...
     * @throws Exception
     */
    protected function setParamsValues($arrayData, $overwrite = false, $logginLevelSet = DUPX_Log::LV_DEBUG)
    {
        ...
        foreach ($arrayData as $key => $arrayValues) {
            if (isset($this->params[$key])) {
                ...
                if ($this->params[$key]->fromArrayData($arrayValues) === false) {
                    DUPX_Log::info('PARAM ISSUE SET KEY['.$key.'] VALUE: '.DupProSnapLibStringU::implodeKeyVals(', ', $arrayValValToStr, '[%s = %s]'));
                    DUPX_Log::info(DUPX_Log::traceToString(debug_backtrace()));
                    // $result = false;
                } else {
                    $log = 'PARAM SET KEY['.$key.']';
                    $log .= (DUPX_Log::isLevel(DUPX_Log::LV_DEBUG) ? (' VALUE: '.DupProSnapLibStringU::implodeKeyVals(', ', $arrayValValToStr, '[%s = %s]')) : '');
                    DUPX_Log::info($log, $logginLevelSet);
                    if ($overwrite) {
    ...
    {
        DUPX_LOG::info("SAVE PARAMS\n".DUPX_Log::traceToString(debug_backtrace()), DUPX_Log::LV_DEBUG);
